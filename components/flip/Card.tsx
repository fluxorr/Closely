...
export function Card({ card, isFlipped, onFlip }: CardProps) {
  return (
    <div
      className="relative w-80 h-[28rem] [perspective:1500px] cursor-pointer group"
      onClick={onFlip}
    >
      <motion.div
      ...
      >
        <div
          className={`absolute inset-0 border-8 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] group-hover:shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] transition-shadow duration-300 rounded-[2rem] flex flex-col items-center justify-center overflow-hidden ${
        ...
        <div
          className={`absolute inset-0 rounded-[2rem] flex flex-col p-8 border-8 border-black shadow-brutal-lg ${
            CATEGORY_COLORS[card.category] || "bg-white text-black"
          }`}
        ...
        >
          <div className="flex justify-between items-center mb-auto w-full">
            ...
            <div className="font-black text-xl bg-white border-4 border-black w-14 h-10 flex items-center justify-center rounded-full shadow-brutal">
              #{card.id}
            </div>
          </div>
